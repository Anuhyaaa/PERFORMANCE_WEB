(function(){'use strict';const sections=document.querySelectorAll('.page-section');const navLinks=document.querySelectorAll('.nav-link, [data-nav]');let currentSection='home';function navigateTo(sectionId){if(sectionId===currentSection)return;sections.forEach(section=>{section.classList.remove('active')});const targetSection=document.getElementById(sectionId);if(targetSection){targetSection.classList.add('active');currentSection=sectionId;updateActiveNavLink(sectionId);history.pushState({section:sectionId},'',`#${sectionId}`);window.scrollTo({top:0,behavior:'smooth'});const sectionLoader=window.ensureSectionScript?window.ensureSectionScript(sectionId):Promise.resolve();sectionLoader.then(()=>{if(currentSection===sectionId){initializeSection(sectionId)}}).catch(()=>{if(currentSection===sectionId){initializeSection(sectionId)}})}}
